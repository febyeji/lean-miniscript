...
private def constructorGoldenCases : List SurfaceGoldenCase :=
  ...
  [
    ...
    ⟨.p2wsh, "hash160(" ++ hash160Text ++ ")",
      "hash160(" ++ hash160Text ++ ")"⟩,
    ⟨.p2wsh, "and_v(v:1,0)", "and_v(v:1,0)"⟩,
    ⟨.p2wsh, "and_b(1,a:1)", "and_b(1,a:1)"⟩,
    ⟨.p2wsh, "or_b(0,a:0)", "or_b(0,a:0)"⟩,
    ⟨.p2wsh, "or_c(0,v:1)", "or_c(0,v:1)"⟩,
    ⟨.p2wsh, "or_d(0,1)", "or_d(0,1)"⟩,
    ⟨.p2wsh, "or_i(1,1)", "or_i(1,1)"⟩,
    ⟨.p2wsh, "andor(0,1,1)", "andor(0,1,1)"⟩,
    ⟨.p2wsh, "a:1", "a:1"⟩,
    ⟨.p2wsh, "s:sha256(" ++ hash256Text ++ ")",
      "s:sha256(" ++ hash256Text ++ ")"⟩,
    ⟨.p2wsh, "c:or_i(pk_k(" ++ key ++ "),pk_k(" ++ key ++ "))",
      "c:or_i(pk_k(" ++ key ++ "),pk_k(" ++ key ++ "))"⟩,
    ⟨.p2wsh, "d:v:1", "dv:1"⟩,
    ⟨.p2wsh, "v:1", "v:1"⟩,
    ⟨.p2wsh, "j:sha256(" ++ hash256Text ++ ")",
      "j:sha256(" ++ hash256Text ++ ")"⟩,
    ⟨.p2wsh, "n:1", "n:1"⟩,
    ⟨.p2wsh, "thresh(1,0)", "thresh(1,0)"⟩,
    ⟨.p2wsh, "multi(1," ++ key ++ ")", "multi(1," ++ key ++ ")"⟩,
    ⟨.tapscript, "multi_a(1," ++ xkey ++ ")",
      "multi_a(1," ++ xkey ++ ")"⟩,
    ⟨.p2wsh, "pk(" ++ key ++ ")", "pk(" ++ key ++ ")"⟩,
    ⟨.p2wsh, "pkh(" ++ key ++ ")", "pkh(" ++ key ++ ")"⟩,
    ⟨.p2wsh, "and_n(0,1)", "and_n(0,1)"⟩,
    ⟨.p2wsh, "t:v:1", "tv:1"⟩,
    ⟨.p2wsh, "l:1", "l:1"⟩,
    ⟨.p2wsh, "u:1", "u:1"⟩
  ]
...
private def normalizationGoldenCases : List SurfaceGoldenCase :=
  ...
  [
    ...
    ⟨.p2wsh, "andor(0,1,0)", "and_n(0,1)"⟩,
    ⟨.p2wsh, "and_v(v:1,1)", "tv:1"⟩,
    ⟨.p2wsh, "or_i(0,1)", "l:1"⟩,
    ⟨.p2wsh, "or_i(1,0)", "u:1"⟩,
...
private def hexadecimalErrorCases : List SurfaceErrorCase :=
  ...
  [
    ...
    ⟨.p2wsh, "j:0", .validationFailed⟩,
    ⟨.p2wsh, "and_v(1,1)", .validationFailed⟩
  ]
