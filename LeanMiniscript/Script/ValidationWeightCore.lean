...
entry in `ValidationWeight` adds runtime limits and metadata checks. This core
module retains the budget-only branch-projection API and its erasure theorem.
-/
